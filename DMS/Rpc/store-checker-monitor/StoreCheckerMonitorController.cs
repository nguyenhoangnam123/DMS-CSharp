...
namespace DMS.Rpc.store_checker_monitor
{
    ...
    public class StoreCheckerMonitorController : RpcController
    {
        ...
        {
            List<StoreCheckerMonitor_StoreCheckerMonitorDTO> StoreCheckerMonitor_StoreCheckerMonitorDTOs = new List<StoreCheckerMonitor_StoreCheckerMonitorDTO>();
            for (int i=0; i<10; i++)
            {
                StoreCheckerMonitor_StoreCheckerMonitorDTO StoreCheckerMonitor_StoreCheckerMonitorDTO = new StoreCheckerMonitor_StoreCheckerMonitorDTO
                {
                    DisplayName = "ABC" + i,
                    OrganizationName = "ORG" + i,
                    Username = "A" + i,
                    StoreCheckings = new List<StoreCheckerMonitor_StoreCheckingDTO>(),
                };
                for (int j=0; j<10; j++)
                {
                    StoreCheckerMonitor_StoreCheckingDTO StoreCheckerMonitor_StoreCheckingDTO = new StoreCheckerMonitor_StoreCheckingDTO
                    {
                        ...
                        External = new HashSet<long>(),
                        Image = new HashSet<long>(),
                        Internal = new HashSet<long>(),
                        Plan = new HashSet<long>(),
                        SalesOrder = new HashSet<long>(),
                        Revenue = new Dictionary<long, long>(),
                    };
                    StoreCheckerMonitor_StoreCheckerMonitorDTO.StoreCheckings.Add(StoreCheckerMonitor_StoreCheckingDTO);
                }    
                StoreCheckerMonitor_StoreCheckerMonitorDTOs.Add(StoreCheckerMonitor_StoreCheckerMonitorDTO);
            }
