...
namespace DMS.Rpc.showing_order
{
    public partial class ShowingOrderController : RpcController
    {
        ...
        [Route(ShowingOrderRoute.BulkDelete), HttpPost]
        public async Task<ActionResult<bool>> BulkDelete([FromBody] List<long> Ids)
        {
            ...
            ShowingOrderFilter = await ShowingOrderService.ToFilter(ShowingOrderFilter);
            ShowingOrderFilter.Id = new IdFilter { In = Ids };
            ShowingOrderFilter.Selects = ShowingOrderSelect.Id;
            ShowingOrderFilter.Skip = 0;
            ShowingOrderFilter.Take = int.MaxValue;

            List<ShowingOrder> ShowingOrders = await ShowingOrderService.List(ShowingOrderFilter);
            ShowingOrders = await ShowingOrderService.BulkDelete(ShowingOrders);
            if (ShowingOrders.Any(x => !x.IsValidated))
                return BadRequest(ShowingOrders.Where(x => !x.IsValidated));
            return true;
        }
        
        [Route(ShowingOrderRoute.Import), HttpPost]
        public async Task<ActionResult> Import(IFormFile file)
        {
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(file.OpenReadStream()))
            {
                ...
                int DateColumn = 4 + StartColumn;
                int ShowingWarehouseIdColumn = 5 + StartColumn;
                int StatusIdColumn = 6 + StartColumn;
                int TotalColumn = 7 + StartColumn;
                ...
                for (int i = StartRow; i <= worksheet.Dimension.End.Row; i++)
                {
                    ...
                    string DateValue = worksheet.Cells[i + StartRow, DateColumn].Value?.ToString();
                    string ShowingWarehouseIdValue = worksheet.Cells[i + StartRow, ShowingWarehouseIdColumn].Value?.ToString();
                    ...
                    ShowingOrder.Code = CodeValue;
                    ShowingOrder.Date = DateTime.TryParse(DateValue, out DateTime Date) ? Date : DateTime.Now;
                    ShowingOrder.Total = decimal.TryParse(TotalValue, out decimal Total) ? Total : 0;
                    AppUser AppUser = AppUsers.Where(x => x.Id.ToString() == AppUserIdValue).FirstOrDefault();
                    ShowingOrder.AppUserId = AppUser == null ? 0 : AppUser.Id;
                    ShowingOrder.AppUser = AppUser;
                    ShowingWarehouse ShowingWarehouse = ShowingWarehouses.Where(x => x.Id.ToString() == ShowingWarehouseIdValue).FirstOrDefault();
                    ShowingOrder.ShowingWarehouseId = ShowingWarehouse == null ? 0 : ShowingWarehouse.Id;
                    ShowingOrder.ShowingWarehouse = ShowingWarehouse;
                    Status Status = Statuses.Where(x => x.Id.ToString() == StatusIdValue).FirstOrDefault();
                    ShowingOrder.StatusId = Status == null ? 0 : Status.Id;
                    ShowingOrder.Status = Status;
                    
                    ShowingOrders.Add(ShowingOrder);
                }
            }
            ShowingOrders = await ShowingOrderService.Import(ShowingOrders);
            if (ShowingOrders.All(x => x.IsValidated))
                return Ok(true);
            else
            {
                ...
                {
                    ShowingOrder ShowingOrder = ShowingOrders[i];
                    if (!ShowingOrder.IsValidated)
                    {
                        string Error = $"Dòng {i + 2} có lỗi:";
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.Id)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.Id)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.Code)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.Code)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.AppUserId)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.AppUserId)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.OrganizationId)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.OrganizationId)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.Date)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.Date)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.ShowingWarehouseId)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.ShowingWarehouseId)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.StatusId)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.StatusId)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.Total)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.Total)];
                        if (ShowingOrder.Errors.ContainsKey(nameof(ShowingOrder.RowId)))
                            Error += ShowingOrder.Errors[nameof(ShowingOrder.RowId)];
                        Errors.Add(Error);
                    }
                }
        ...
        [Route(ShowingOrderRoute.ExportTemplate), HttpPost]
        public async Task<ActionResult> ExportTemplate([FromBody] ShowingOrder_ShowingOrderFilterDTO ShowingOrder_ShowingOrderFilterDTO)
        {
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
            
            string path = "Templates/ShowingOrder_Template.xlsx";
            ...
            return File(output.ToArray(), "application/octet-stream", "ShowingOrder.xlsx");
        }
